...
export default function ImportModal({ onImport, onClose }) {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      else if (tab === 'text' && text.trim()) fd.append('text', text.trim())
      else { setError('Add a file or paste some text first.'); setLoading(false); return }
      ...
      onImport({ profile: data.profile, warnings: data.warnings || [] })
      onClose()
    } catch (e) {
      ...
      <div className="bg-white w-full max-w-lg rounded-t-3xl md:rounded-2xl overflow-hidden shadow-2xl flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
          <div>
            ...
            <p className="text-sm text-gray-400 mt-0.5">We'll read your document and let you review before saving</p>
          </div>
              ...
              {file ? (
                <>
                  ...
                  <button onClick={e => { e.stopPropagation(); setFile(null) }}
                    className="text-xs text-gray-400 hover:text-gray-600 mt-2 underline">Remove</button>
                </>
              ) : (
                <>
