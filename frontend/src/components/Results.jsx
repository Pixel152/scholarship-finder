...
function ScholarshipCard({ s, saved = false, onSave, onUnsave, index = 0 }) {
  ...
  return (
    ...
    >
      ...
      <div className="p-5 md:p-6">
        ...
        <div className="flex items-start justify-between gap-4 mb-4">
          <div className="min-w-0">
            <p className="text-xs text-gray-400 font-semibold mb-1 tracking-wide">#{s.rank}</p>
            <h3 className="text-base md:text-lg font-bold text-gray-900 leading-snug">{s.name}</h3>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <button
              ...
              className={`p-2 rounded-xl transition-all duration-150 press-effect ${
                saved
        ...
        <div className="flex flex-wrap items-center gap-2 mb-4">
          ...
          {s.amount && (
            <span className="text-sm font-semibold text-gray-800">
              {s.amount}
              {s.renewable && <span className="text-gray-400 font-normal ml-1">· renewable</span>}
            </span>
          )}
        </div>
        ...
        <div className="space-y-2.5 border-t border-gray-50 pt-4">
          ...
          {s.scoreReason && (
            <div className="flex gap-3 text-sm">
              <span className="flex-shrink-0 w-28 text-gray-400 leading-relaxed">Score reason</span>
      ...
      {(s.applyUrl || s.applyContact) && (
        <div className="px-5 md:px-6 py-3.5 bg-gray-50 border-t border-gray-100 flex items-center justify-between gap-4">
          <span className="text-xs text-gray-400 truncate min-w-0">{s.applyContact}</span>
  ...
  return (
    <div className="min-h-screen">
      <div className="bg-white border-b border-gray-100 px-6 py-4">
        <h1 className="font-bold text-gray-900">Results (raw)</h1>
      </div>
...
export default function Results({ output, onProfile, onNewSearch, trackerIds = new Set(), onSave, onUnsave }) {
  ...
  if (scholarships.length === 0) return <RawView output={output} />
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50">
      {/* Sticky top bar */}
      <div className="bg-white border-b border-gray-100 sticky top-0 z-10 px-4 md:px-6 py-4">
        <div className="max-w-4xl mx-auto flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 min-w-0">
            <h1 className="font-bold text-gray-900 text-base flex-shrink-0">Results</h1>
            <span className="bg-emerald-50 text-emerald-700 text-xs font-semibold px-2.5 py-1 rounded-full border border-emerald-200 flex-shrink-0">
              {scholarships.length} found
            </span>
          </div>
          <div className="flex gap-2 flex-shrink-0">
            <button onClick={download} className="hidden sm:block text-sm text-gray-600 border border-gray-200 px-3 py-2 rounded-lg hover:bg-gray-50 transition-colors font-medium">
              Download
            </button>
            <button onClick={onNewSearch} className="text-sm bg-blue-600 text-white px-3 md:px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors font-medium">
              New Search
            </button>
          </div>
        </div>
      ...
      <div className="max-w-4xl mx-auto px-4 md:px-6 py-6 md:py-8 w-full">
        ...
        <div className="grid grid-cols-3 gap-2 md:gap-4 mb-6 md:mb-8">
          {[
            { value: scholarships.length, label: 'scholarships found', color: 'text-gray-900',    delay: 0 },
            { value: openCount,           label: 'open right now',     color: 'text-emerald-600', delay: 60 },
            { value: totalValue || '—',   label: 'estimated value',    color: 'text-blue-600',    delay: 120 },
          ].map(({ value, label, color, delay }) => (
            <div key={label}
              className="bg-white rounded-xl p-3 md:p-5 border border-gray-100 shadow-card text-center transition-all duration-200 hover:shadow-card-lg animate-fade-up"
              style={{ animationDelay: `${delay}ms` }}
            >
              <div className={`text-xl md:text-3xl font-bold tabular-nums ${color}`}>{value}</div>
              <div className="text-[11px] md:text-sm text-gray-400 mt-1 leading-tight">{label}</div>
            </div>
          ))}
        </div>
        ...
        {/* Quick wins */}
        {quickWins && (
          <div className="mt-8 bg-gradient-to-br from-amber-50 to-orange-50 rounded-2xl p-5 md:p-6 border border-amber-100 animate-fade-up">
